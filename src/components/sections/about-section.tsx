
export function AboutSection() {
  return (
    <section className="pt-0 pb-8 md:pb-12 container mx-auto px-4 sm:px-6 lg:px-8" id="about">
      <div className="glass-card animated-outline max-w-5xl mx-auto p-8 md:p-12 rounded-2xl">
        <h2 className="font-headline text-3xl font-bold tracking-tight text-foreground sm:text-4xl text-center">
          Let the Fox Handle It
        </h2>
        <div className="mt-6 space-y-4 text-foreground/80">
          <p>
            Automation shouldn’t feel complicated or corporate. At Foxxage, we make it clever, simple, and built just for you.
          </p>
          <p>
            Our AI Agents and workflow automations take care of the busywork — connecting your tools, answering customers on WhatsApp, and turning data into decisions — so you can stay focused on growing your business.
          </p>
        </div>
      </div>
    </section>
  );
}
